...
def get_mnist(net):
    ...
    Y_train = keras.utils.to_categorical(y_train, nb_classes).astype('float32')
    Y_test  = keras.utils.to_categorical(y_test, nb_classes).astype('float32')
...
def get_svhn(net):
    ...
    d1 = loadmat('/LOCAL/gjin/iclr/datasets/train_32x32.mat')
    X_train = np.transpose(d1['X'])
    y_train = d1['y']%10 
    d1 = loadmat('/LOCAL/gjin/iclr/datasets/test_32x32.mat')
    X_test = np.transpose(d1['X'])
    y_test = d1['y']%10
    print(X_train.shape)
...
def do_report(epoch):
    if epoch < 10:
        ...
        return True
    elif epoch < 10000 and epoch%1000==0:
        return True
